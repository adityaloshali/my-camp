    ...
    if (req.isAuthenticated()) {
        ...
        Campground.findById(req.params.id, function (err, foundCampground) {
            if (err) {
                ...
                if (foundCampground.author.id.equals(req.user._id)) {     //.equals because mongoose returns object , not string
                    next();
                } else {
...
middlewareObj.checkCommentOwnership = function (req, res, next) {
    ...
    if (req.isAuthenticated()) {
        ...
        // send flash notification to user to log in first
        req.flash("error", "You need to be logged in to do that"); //sends a non-persistant message
        res.redirect("back");
    }
}
